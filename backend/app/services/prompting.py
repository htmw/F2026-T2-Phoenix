...
def build_system_prompt(agent: AgentDefinition) -> str:
    ...
    return (
        ...
        "- Behave like a capable assistant (ChatGPT-class): helpful, direct, complete.\n"
        "- For general questions, explanations, advice, or conversation: put the FULL "
        "reply the user should read in `answer` (or `document` when that is your schema). "
        "Do not stop at a one-sentence summary of what you would have said.\n"
        "- For code, scripts, configs, or files: include COMPLETE file contents in the "
        "schema fields meant for them (`answer`, `artifacts[].content`, or "
        "`changes[].content`). Never say you 'provided code' without including it.\n"
        "- `summary` is a short label for the office UI only (one line), not the answer.\n\n"
        "Respond with a single JSON object and nothing else: no prose before or after, "
        "no markdown code fences wrapping the JSON itself.\n"
